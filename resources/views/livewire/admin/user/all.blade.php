    ...
    <div class="flex">
        <div class="lg:w-2/5 md:w-2/5 w-full">
            <div class="mb-4 flex gap-4">
                <input wire:model.live.debounce.500ms="search" type="search" class="form-input px-4 py-3 w-full rounded-none border-gray-400 font-thin text-xl placeholder:text-gray-300 placeholder:text-sm focus:outline-none focus:ring-0 active:ring-gray-100" placeholder="Search User">
                <a href="{{ route('admin.users.new') }}" class="bg-black/80 hover:bg-black/70 dark:bg-boxdark whitespace-nowrap rounded-none text-xs inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent font-thin text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Add New
                </a>
            </div>
        </div>
            ...
            <table class="w-full table-auto">
                <thead class="pt-6 border-y-2 border-stone-200 dark:border-stone-600">
                    <tr class="bg-gray-2 text-left dark:bg-meta-4">
                        ...
                        <th class="min-w-[120px] px-4 py-7 font-medium text-black dark:text-white">
                            Status
                            <p class="foont-thin text-xs italic">(Click to toggle and change user status)</p>
                        </th>
                ...
                <tbody>
                    @foreach ($this->users as $key => $user )
                        <tr wire:key="{{$user->id}}" class="dark:hover:bg-gray-3 dark:hover:bg-opacity-10 hover:bg-gray-2">
                            <td class="border-b border-[#eee] p-4 pl-9 dark:border-strokedark xl:pl-11">
                                <h5 class="font-medium text-black dark:text-white">{{ $user->firstname }} {{ $user->lastname }}</h5>
                                <p class="text-xs text-slate-500 dark:text-slate-300 italic">{{ $user->email }}</p>
                            </td>
                            <td class="border-b border-[#eee] p-4 dark:border-strokedark">
                                @foreach ( $user->partners as $key => $partner)
                                    <p wire:key="{{ $key }}" class="capitalize inline-flex rounded-full bg-warning bg-opacity-20 px-3 py-1 text-xs font-thin text-warning">
                                        {{ $partner->name}}
                                    </p>
                                @endforeach
                            </td>
                            <td class="border-b border-[#eee] p-4 dark:border-strokedark">
                                <p wire:confirm="Are you sure you want this changes?" wire:click="toggleActive({{$user}})" class="inline-flex rounded-full px-2 py-1 shadow drop-shadow text-xs bg-opacity-40 cursor-pointer hover:bg-opacity-100 hover:-translate-y-0.5 duration-300 border {{ $user->is_active ? 'bg-green-500 border-green-600 text-green-700 dark:text-green-400 hover:text-green-100 dark:hover:bg-green-600 dark:hover:text-green-200' : 'bg-red-500 border-red-600 text-red-700 dark:text-red-400 dark:hover:bg-red-600 dark:hover:text-red-200 hover:text-red-200' }}">
                                    @if ($user->is_active)
                                        Active
                                    @else
                                        Inactive
                            ...
                            <td class="border-b border-[#eee] p-4 dark:border-strokedark">
                                <div class="flex justify-end items-center space-x-3.5">
                                    <a wire:navigate href="{{ route('admin.users.edit', $user->id) }}" class="hover:text-primary transform hover:-translate-y-1 duration-300 drop-shadow">
                                    ...
                                    <button wire:confirm="Are you sure you want to delete this user?" wire:click="delete({{$user}})" class="hover:text-danger transform hover:-translate-y-1 duration-300 drop-shadow"> 
                                        <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
