...
class RegistrationFormModal extends Component
{
    ...
    public $reviewRegistration = false;
    ...
    public function getProgramItem($programCode, $programType)
    {
        ...
        $this->requiredFields = ['email']; // FOR TESTING
    ...
    {
        $this->form->confirmAndProceedToPayment();
    }
    ...
    public function computeNetTotal()
    {
        ...
        {
            return $this->form->finalRegistrationData['programItem']['price'] - $this->form->finalRegistrationData['promoDetails']['price'];
        }
        else
        {
            return $this->form->finalRegistrationData['programItem']['price'];
        }
    }

    public function backToRegistrationForm()
    {
        if ($this->form->finalRegistrationData) 
        {
            $this->form->fields = $this->form->finalRegistrationData['registrationFields'];
            $this->form->promoCodeDetails = $this->form->finalRegistrationData['promoDetails'];
            $this->form->promoCode = $this->form->promoCodeDetails['promocode'] ?? '';
            
            // Emit events to update each field component
            foreach ($this->form->fields as $key => $value) 
            {
                $this->dispatch('set-field-value', ['key' => $key, 'value' => $value]);
            }
            
            // Handle extra fields if they exist
            if (isset($this->form->finalRegistrationData['extraFieldValues'])) 
            {
                $extraFieldValues = json_decode($this->form->finalRegistrationData['extraFieldValues'], true);
                foreach ($extraFieldValues as $field) 
                {
                    $key = array_key_first($field);
                    $this->form->fields[$key] = $field[$key];
                    $this->dispatch('set-field-value', ['key' => $key, 'value' => $field[$key]]);
                }
            }
        }

        $this->reviewRegistration = false;
    }
